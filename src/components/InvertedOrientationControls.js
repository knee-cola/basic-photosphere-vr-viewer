...
export function InvertedOrientationControls( object ) {
	...
	var setObjectQuaternion = function() {
		...
		return function( quaternion, alpha, beta, gamma, orient ) {
			...
			rotQ.setFromEuler( euler ); // orient the device

			rotQ.multiply( q1 ); // camera looks out the back of the device, not the top

			rotQ.multiply( q0.setFromAxisAngle( zee, - orient ) ); // adjust for screen orientation
	...
	this.update = function() {
		...
		return(setObjectQuaternion( scope.object.quaternion, alpha, beta, gamma, orient ));
	};
